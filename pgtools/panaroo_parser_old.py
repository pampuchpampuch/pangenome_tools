...
class PanarooCsvInfo(BaseSeq):
    """
    Represent gene as defined in gene_data.csv in panaroo output
    """
    def __init__(self, gff_file, scaffold_name, clustering_id, annotation_id, refound, seq = None):
        self.gff_file = gff_file
        self.scaffold_name = scaffold_name
        self.chr_name = gff_file + "." + scaffold_name
        self.clustering_id = clustering_id
        self.annotation_id = annotation_id
        self.refound = refound
        self.seq = seq
...
class PanarooGene(BaseSeq):
    ...
    def __init__(self, seq_name: str, start: int, end: int, strand: int, src_size: int, is_refound: bool, annotation_id: str, in_format="maf"):
        super().__init__(seq_name, start, end, strand, src_size, in_format)
        self.refound = is_refound
        self.annotation_id = annotation_id    
...
class PanarooCluster(SeqCollection):
    ...
    def __init__(self, id: int, seq_dict: SeqIO.Dict[str, BaseSeq]):
        super().__init__(id, seq_dict)
        # self.cluster_name = cluster_name
        # self id is clustering_id
...
class Panaroo(Pangenome):
    ...
    """
    def __init__(self, seqs_collections: list[SeqCollection], graph_structure: nx.Graph):
    ...
    with open(gene_data_path) as f:
        ...
        for line in f:
            if line:
                ...
                genes_dict[clustering_id] = PanarooCsvInfo(gff_file, scaffold_name, clustering_id, annotation_id)
